    ...
    node {		
		try {
			...
			if (env.BRANCH_NAME != 'master') {
				...
				deleteDir()
				withEnv(['PIPELINE_VERSION='+version,'IMAGE_NAME='+config.imageName,'CONTAINER_NAME='+config.containerName,'OUTPUT_PATH=build','SLN_FILE='+config.slnFile], 'DOTNET_VERSION='+config.dotnetVersion, 'DOTNET_SDK_VERSION='+config.dotnetSdkVersion) {
					timestamps {
						checkout()
						prepareScripts()
						build()
